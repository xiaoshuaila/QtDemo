    ...
    _fileDbg(NULL)
{
    ...
    */
    _locationFilterTypes << "无" << "平均移动" << "平均·指数移动";
    _usingFilter = 0;
...
//    socket = NULL;
//    QObject::connect(server,&QTcpServer::newConnection,this,&RTLSClient::Slot_server_New_Connect);
}
...
void RTLSClient::onConnected(QString ver, QString conf)
{
    ...
    qDebug() << "RTLSClient.cpp onReady 2";
...
{
//    ViewSettingsWidget *ciew=new ViewSettingsWidget;
//    QObject::connect(ciew,SIGNAL(sendchuandi(int)),this,SLOT(slotchuandi(int)));
     QByteArray data;
     QString str;
    //qDebug()<<asf;
    //qDebug()<<Name_Of_ComPort;
   ...
   // qDebug()<<asf;
    ...
    while(length >= TOF_REPORT_LEN) //we have received a range report from an anchor or a tag
    {
        ...
        {
#if 1
            QByteArray header_1 = data.mid(offset, 1);

            if(header_1.contains("m")) //查找首字符
            {
                ...
                if(header.contains("mr")) //loop here until we reach header ("mc")
                {
...
void RTLSClient::newData1()
{

    //qDebug() << "Add new Anchor: 0x04";
#if 1
   // QByteArray data = NULL;
    QByteArray data = soc1;
   // emit Signal_Client_Recvdata(data);
    //qDebug()<<socket->readAll();

    //QByteArray data =   socket->read(TOF_REPORT_LEN * 20);
    QDateTime now = QDateTime::currentDateTime();
    QString nowstr = now.toString("T:hhmmsszzz:");
    int length = data.length();
    int offset = 0;
    ...
    while(length >= TOF_REPORT_LEN) //we have received a range report from an anchor or a tag
    {
        ...
        //qDebug() << "current_len1 "<< length << offset << data.mid(offset, length);
        while(length >= TOF_REPORT_LEN)
        {
            QByteArray header_1 = data.mid(offset, 1);
...
void RTLSClient::trilaterateTag(int tid, int seq, int idx)
{
    ...
    if(count >= 3)
    {
        ...
        if(calculateTagLocation(&report, count, &rp.rangeValue[lastSeq][0] ) == TRIL_3SPHERES)
        {
            ...
            if(_file)
            {
                QString s = nowstr + QString("LE:%1:%2:[%3,%4,%5]:").arg(tid).arg(rp.numberOfLEs)./*arg(lastSeq).*/arg(report.x).arg(report.y).arg(report.z) +
                        QString("%1:%2:%3:%4").arg(rp.rangeValue[lastSeq][0]).arg(rp.rangeValue[lastSeq][1]).arg(rp.rangeValue[lastSeq][2]).arg(rp.rangeValue[lastSeq][3])+":"+nowroomnane+"\n";
                QTextStream ts( _file );
                ts << s;
                qDebug()<<nowroomnane;
            }
...
//开始。。。。。。。。。
//void RTLSClient::Slot_server_New_Connect()
//{
//    socket = server->nextPendingConnection();

//    QObject::connect(socket,&QTcpSocket::readyRead, this, &RTLSClient::newData1);//jerry add
//    QObject::connect(socket, &QTcpSocket::disconnected, this, &RTLSClient::Slot_socket_Disconnected);
//    QMessageBox::about(NULL, "Socket", "TCP client connection successful");
//    emit Signal_Client_Connect();
//}

//void RTLSClient::Slot_socket_Disconnected()
//{
//    emit Signal_Client_Disconnect();
//}

//void RTLSClient:: Slot_button_connect(int port, bool is_open)
//{
//    qDebug()<< "Slot_button_connect!" << port << is_open;
...
//            return ;
//        }
//        emit Signal_Sever_State(true);
//    }
//    else
//    {
//        //socket->abort();
//        server->close();
//        emit Signal_Sever_State(false);
//    }
//#endif
//}

//void RTLSClient::Slot_button_send(QByteArray buf)
//{
//    socket->write(buf);
//    socket->flush();
//}

//void RTLSClient::chuandi(QTcpServer *ser, int a)
//{
//    asf=250;
//    qDebug()<<a;
//}
void RTLSClient::slotchuandi(QByteArray soc, int a)
{
    //socket->readAll()=soc;
    soc1=soc;
    newData1();
