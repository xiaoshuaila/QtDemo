...
void GraphicsWidget::onReady()
{
    ...
    //liwenfneg
    QObject::connect(this, SIGNAL(sendpoint(QPointF)), graphicsView(), SLOT(readpoint(QPointF)));
    ...
    while (ui->tagTable->rowCount())
    {
        ...
        if(item)
        {
            ...
            if(tag->tagLabel) //remove label
            {
                ...
                tag->Tagc->setOpacity(0); //hide it
                this->_scene->removeItem(tag->Tagc);
                delete(tag->Tagc);
                tag->Tagc = NULL;
            }
...
void GraphicsWidget::anchorTableClicked(int r, int c)
{
    ...
    if(c == ColumnID) //toggle label
    {
        ...
        anc->c->setOpacity(anc->show ? 1.0 : 0.0);
...
void GraphicsWidget::tagTableClicked(int r, int c)
{
    ...
    if(c == ColumnID) //toggle label
    {
       ...
       // tag->Tagc->setOpacity(tag->showLabel ? 1.0 : 1.0);
    }
...
void GraphicsWidget::addNewTag(quint64 tagId)
{
    ...
    //add text label and hide until we see if user has enabled showLabel
    {
        ...
        QPixmap pixmap0( "://color.png" );
        QGraphicsPixmapItem *tagc=this->_scene->addPixmap(QPixmap(pixmap0));
        tagc->setPixmap(QPixmap("://renwu.png"));
        tagc->setScale(0.0003);
        tagc->setRotation(180);
        tag->Tagc=tagc;
        tag->Tagc->setOpacity(0);
    }
}
/**
 * @fn    tagPos
...
void GraphicsWidget::tagPos(quint64 tagId, double x, double y, double z)
{
    ...
    else
    {
        ...
        {
            float a=_tagSize*0.1;
            QAbstractGraphicsShapeItem *tag_pt = this->_scene->addEllipse(-1*a/2, -1*a/2, a, a);
  ...
  //      ViewSettingsWidget *v=new ViewSettingsWidget;
        QPointF cen;
        int zong=zhui;
        if(zong!=0)
        {
        QMap<quint64, Tag*>::iterator i = _tags.find(taghao);
        if(i != _tags.end())
            {
            cen=_tags.value(taghao,NULL)->tagLabel->pos();
            if(tagId==taghao)
            {
                if(poin[poinnum].rx()==0)
                {
                poin[poinnum]=_tags.value(taghao,NULL)->tagLabel->pos();
                poin[poinnum].rx()=poin[poinnum].rx()-0.15;
                poin[poinnum].ry()=poin[poinnum].ry()-0.15;
                }
                poinnum++;
            }
            if (poinnum>50000)
        ...
        tag->Tagc->setPos(x+0.04,y+0.04);
        tag->p[tag->idx]->setPos(x, y);
        tag->Tagc->setOpacity(1);
        if(_showHistory)
        {
            tagHistory(tagId);
...
void GraphicsWidget::anchPos(quint64 anchId, double x, double y, double z, bool show, bool update)
{
    ...
    else
    {
        ...
        if (anc->a == NULL)
        {
            ...
            anch->setBrush(QBrush(QColor::fromRgb(85, 60, 150, 0)));
            anch->setToolTip("0x"+QString::number(anchId, 16));
            anc->a = anch;

            QPixmap pixmap0( "://color.png" );
            QGraphicsPixmapItem *cnch=this->_scene->addPixmap(QPixmap(pixmap0));
            cnch->setPixmap(QPixmap("://zuobiao.png"));
            //cnch->setFlags(QGraphicsItem::ItemIgnoresTransformations);
            cnch->setScale(0.00015);
            //cnch->setFlag(QGraphicsItem::ItemIgnoresTransformations);
            cnch->setRotation(180);
            anc->c=cnch;
        }
...
void GraphicsWidget::zhuizong(int a,int b)
{
    zhui=a;
    taghao=b;
    qDebug()<<"af";
}
void GraphicsWidget::cleartag(){
    qDebug()<<"hhh";
    clearTags();
}
