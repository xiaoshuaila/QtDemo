...
void GraphicsView::drawOrigin(QPainter *painter)
{
    ...
    int i=poinnum+1;
        ...
        if(!(poin[i-2].rx()==poin[i-1].rx()&&poin[i-2].ry()==poin[i-1].ry()))
            if(!(poin[i-1].rx()==0&&poin[i-1].ry()==0||poin[i-2].rx()==0&&poin[i-2].ry()==0))
        painter->drawLine(poin[i-1],poin[i-2]);
        //qDebug()<<poin[i-1];
        i=i-1;
        }
        translateView(0,0);
...
//liwnfeng
void GraphicsView::readpoint(QPointF cen)
{
    cen1.rx()=cen2.rx()-cen.rx();
    cen1.ry()=cen2.ry()-cen.ry();
    //translateView(-cen1.rx(),-cen1.ry());
    cen2.rx()=cen.rx();
    cen2.ry()=cen.ry();
}
